...
class Listings:
    ...
    def latetest_start(start=1, limit=100, convert="USD", sort="volume_24h", sort_dir="desc", market_type="all", convert="USD"):
...
class Quotes:
    ...
    def historical_slug(self, slug: str, time_start: Union[datetime, float], time_end: Union[datetime, float], count=10, interval="5m", convert="USD"):
        return self.request("historical", locals())

    def latest_id(self, id: Union[list, str, int], convert="USD"):
        return self.request("latest", locals())

    def historical_slug(self):
        return self.request("latest", locals())
